...
bname brand.bname@site_link %type := '&brandname' ;
value expense.category@site_link %type;
a customer.cid@site_link %type := &id;
customername customer.cname@site_link %type;
cnt number :=0;
cost number :=0;
flag number :=0;
...
if (a = 0) then
	...
	for r in (select bid,bname,income,cid from brand@site_link) loop
...
for r in (select cname from customer@site_link where cid=a) loop
